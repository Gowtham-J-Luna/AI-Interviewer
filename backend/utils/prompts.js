const questionAnswerPrompt = (
    ...
    resumeData = {}
        ...
        ${jobDescription ? `- Job Description: Match the emphasis of this role where relevant:\n"""${jobDescription}"""\n` : ""}
        ${resumeData?.summary || (resumeData?.skills && resumeData.skills.length)
            ? `- Parsed Resume Snapshot:
        Summary: ${resumeData.summary || "N/A"}
        Inferred Role: ${resumeData.inferredRole || "N/A"}
        Inferred Experience: ${resumeData.inferredExperienceYears || 0} years
        Skills: ${(resumeData.skills || []).join(", ") || "N/A"}
        Experience Highlights: ${(resumeData.experience || []).join(" | ") || "N/A"}
        Project Highlights: ${(resumeData.projects || []).join(" | ") || "N/A"}
        Education: ${(resumeData.education || []).join(" | ") || "N/A"}
        Top Keywords: ${(resumeData.topKeywords || []).join(", ") || "N/A"}`
            : ""}
        - Write ${numberOfQuestions} interview questions.
        - Prioritize resume-specific questions first, then job-description-specific questions, then broad fundamentals.
