...
const CoachMateDrawer = ({ isOpen, onClose }) => {
    ...
    const voiceMessageTimeoutRef = useRef(null);
    const isSendingRef = useRef(false);
    const lastSubmittedRef = useRef({ text: "", timestamp: 0 });

    const normalizeMessage = (value = "") => value.replace(/\s+/g, " ").trim();

    const shouldSuppressMessage = (rawMessage, isVoiceInput = false) => {
        const normalized = normalizeMessage(rawMessage);
        if (!normalized) return true;

        const { text: lastText, timestamp } = lastSubmittedRef.current;
        const ageMs = Date.now() - timestamp;
        const lastNormalized = normalizeMessage(lastText);

        if (normalized.toLowerCase() === lastNormalized.toLowerCase() && ageMs < 15000) {
            return true;
        }
        ...
        if (
            ...
            ageMs < 12000 &&
            lastNormalized &&
            (normalized.toLowerCase().startsWith(lastNormalized.toLowerCase()) ||
                lastNormalized.toLowerCase().startsWith(normalized.toLowerCase()))
        ) {
            return true;
        }
    ...
    const queueVoiceSubmission = (transcript) => {
        const normalized = normalizeMessage(transcript);
        if (!normalized) return;

        if (voiceMessageTimeoutRef.current) {
            clearTimeout(voiceMessageTimeoutRef.current);
        }

        voiceMessageTimeoutRef.current = setTimeout(() => {
            handleSend(normalized, { isVoiceInput: true });
        }, 900);
    };
    ...
    useEffect(() => {
        ...
        if (SpeechRecognition) {
            ...
            recognition.onresult = (event) => {
                ...
                if (transcript) {
                    queueVoiceSubmission(transcript);
                }
            };
        ...
        return () => {
            if (voiceMessageTimeoutRef.current) {
                clearTimeout(voiceMessageTimeoutRef.current);
                voiceMessageTimeoutRef.current = null;
            }
        };
    }, [isVoiceMode]);
    ...
    const handleSend = async (forcedMessage = null, options = {}) => {
        const normalizedText = normalizeMessage(forcedMessage || input);
        const isVoiceInput = !!options.isVoiceInput;

        if (!normalizedText || loading || isSendingRef.current || shouldSuppressMessage(normalizedText, isVoiceInput)) {
            return;
        }
        ...
        lastSubmittedRef.current = { text: normalizedText, timestamp: Date.now() };

        const userMsg = { role: "user", content: normalizedText, timestamp: new Date() };
        setMessages((prev) => [...prev, userMsg]);
        setInput("");
        setLoading(true);

        try {
            const response = await axiosInstance.post("/api/coach/chat", { message: normalizedText });
                ...
                >
                    ...
                    <div className="flex items-center justify-between px-5 py-4 relative z-10">
                        ...
                        <div className="flex items-center gap-1">
                            ...
                            <button onClick={handleNewSession} className="p-2 text-white/20 hover:text-white transition-colors cursor-pointer" title="Start Fresh"><Add01Icon size={18} /></button>
                        ...
                        {initialLoading ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center relative">
                                ...
                                >
                                    ...
                                    <h2 className="text-xl font-bold text-white mb-2">Hey, I'm AI Coach</h2>
                                    ...
                                    {isVoiceMode ? (
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <span className="text-xs font-bold text-white/40 uppercase tracking-widest">
                                                {isListening ? "Listening..." : isSpeaking ? "AI Coach is speaking" : "Tap to Speak"}
                                            </span>
                    ...
                    <div className="p-4 relative z-10">
                                ...
                                {(isSpeaking || isListening) ? (
                                    <div className="flex flex-col items-center justify-center py-3 px-4 rounded-2xl">
                                        ...
                                        <span className="text-[9px] uppercase tracking-[0.15em] font-black text-white/40 mt-2">
                                            {isSpeaking ? "AI Coach Speaking" : "AI Coach Listening"}
                                        </span>
                                    </div>
                                ) : (
                                    <button
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-bold tracking-tight">Tap to speak to AI Coach</span>
                                    </button>
                                )}
                            </div>
                        )}
                        ...
                        <div className="relative group">
                            <input
                                ...
                                placeholder={isVoiceMode ? "Voice mode active..." : "Message AI Coach..."}
