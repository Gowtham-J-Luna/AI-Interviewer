...
const InterviewDashboard = ({ analysis, sessionStats, currentQuestion, transcript }) => {
  ...
  const derivedSkillsData = useMemo(() => {
    const normalizedScore = Math.max(0, Math.min(10, scoreValue));
    const transcriptWords = transcript ? transcript.trim().split(/\s+/).filter(Boolean).length : 0;
    const strengthsCount = analysis?.strengths?.length || 0;
    const improvementsCount = analysis?.improvements?.length || 0;
    const takeawayCount = analysis?.keyTakeaways?.length || 0;
    ...
    return [
      {
        skill: 'Clarity',
        score: clamp(normalizedScore + (strengthsCount >= 2 ? 0.6 : 0.2) - improvementsCount * 0.15),
      },
      {
        skill: 'Structure',
        score: clamp(normalizedScore + takeawayCount * 0.25 - improvementsCount * 0.1),
      },
      {
        skill: 'Confidence',
        score: clamp(normalizedScore + (transcriptWords >= 90 ? 0.5 : transcriptWords >= 45 ? 0.2 : -0.3)),
      },
      {
        skill: 'Relevance',
        score: clamp(normalizedScore + (analysis?.refinedAnswer ? 0.35 : 0) - improvementsCount * 0.08),
      }
    ];
  }, [analysis, scoreValue, transcript]);
  const scoreData = [
    { name: 'Your Score', value: scoreValue, fill: '#FFFFFF' },
    { name: 'Remaining', value: 10 - scoreValue, fill: '#374151' }
  ];

  const skillsData = analysis?.metrics
    ? [
        { skill: 'Clarity', score: analysis.metrics.clarity || 0 },
        { skill: 'Structure', score: analysis.metrics.structure || 0 },
        { skill: 'Confidence', score: analysis.metrics.confidence || 0 },
        { skill: 'Relevance', score: analysis.metrics.relevance || 0 }
      ]
    : derivedSkillsData;
