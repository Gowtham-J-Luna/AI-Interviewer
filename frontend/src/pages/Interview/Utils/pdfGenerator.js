  ...
  try {
    ...
    const addPageHeader = () => {
      if (pdf.internal.getCurrentPageInfo().pageNumber > 1) {
        ...
        pdf.text(APP_BRAND.name, margin, 15);
    ...
    pdf.setTextColor(...colors.accent);
    pdf.setFont('helvetica', 'bold');
    pdf.text(APP_BRAND.name, margin, 18);
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.setFontSize(9);
      pdf.setTextColor(...colors.accent);
      pdf.text(APP_BRAND.name, margin, pageHeight - 7);

      pdf.setTextColor(200, 200, 200);
      pdf.text(`${APP_BRAND.shortName} - Professional Analysis Report`, margin + 35, pageHeight - 7);
